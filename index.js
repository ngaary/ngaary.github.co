...
function ajoutVideo(stream, userId) {
    ...
    if (!existingVideo) {
        ...
        video.id = `video-${userId}`; // Correction de la concaténation de l'ID
...
function addScreenShare() {
    ...
    navigator.mediaDevices.getDisplayMedia({ video: { cursor: "always" }, audio: true })
        .then((screenStream) => {
            ...
            call.on('stream', function(remoteStream) {
                let userScreenVideoId = `video-screen-${name}`; // Correction de l'ID du partage d'écran
                ...
                if (!document.getElementById(userScreenVideoId)) {
                    ajoutVideo(remoteStream, userScreenVideoId); // Ajoute la vidéo de l'invité avec l'ID du partage d'écran
                }
            });
